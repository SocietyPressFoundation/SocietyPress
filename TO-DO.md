...
- [ ] Make the `societypress/SocietyPress` GitHub repo public (or create it if it doesn't exist yet)
...
- [x] Bundle de-branded — every reference to SAGHS / txsaghs removed from the shippable plugin and parent theme. Theme registry entry deleted, "SAGHS Publication" acq_code option replaced with "Society Publication", placeholders generalized, San Antonio kept in the Central Time label (plus Houston, Dallas, Austin, Chicago) since it's a city, not a society. SAGHS-specific work for txsaghs.com lives separately in its own child theme.
...
- [x] SAGHS logo redo — Charles re-did it on 2026-04-14
...
- [x] Orphaned theme cleanup — removed five duplicate `societypress-*` folders + stale `saghs` and `san-antonio-genealogical-historical-soci` from local working copy.
