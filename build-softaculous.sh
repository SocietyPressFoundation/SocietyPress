...
# ---- Copy child themes (all generic ones, NOT society) ----
echo "Copying child themes..."
for CHILD_DIR in "$SCRIPT_DIR"/theme-*/; do
    CHILD_NAME=$(basename "$CHILD_DIR")
    # Skip the society — that's site-specific, not for distribution
    if [ "$CHILD_NAME" = "theme-society" ]; then
...
if [ -d "$BUILD_DIR/wp-content/themes/society" ] || [ -d "$BUILD_DIR/wp-content/themes/theme-society" ]; then
    echo "  ERROR: the society child theme found in build. Removing..."
    rm -rf "$BUILD_DIR/wp-content/themes/society" "$BUILD_DIR/wp-content/themes/theme-society"
fi
