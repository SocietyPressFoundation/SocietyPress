...
# Deploys plugin and/or themes to demo.getsocietypress.org and txsaghs.com.
#
# Usage:
...
#
# WHY the structure: each target deploys to BOTH active sites (demo + txsaghs)
...
# demo doesn't prevent the txsaghs deploy from completing).
OVERALL_STATUS=0
...
# ----------------------------------------------------------------------------
# Both-sites wrappers. These call the per-site helpers for demo + txsaghs so
...
deploy_theme_all_sites() {
    ...
    deploy_theme_to "$local_dir" "$theme_name" "$SAGHS_BASE" "txsaghs.com"
}
