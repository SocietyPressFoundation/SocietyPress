 ...
 *
 * Serves plugin ZIP files with license authentication.
 *
 * WHY: WordPress downloads updates from here after token validation.
 */
...
// Build file path
$file_path = __DIR__ . '/../../../../versions/societypress-' . $version_info['version'] . '.zip';
...
if ( ! file_exists( $file_path ) ) {
	...
	error_log( 'SocietyPress download: File not found - ' . $file_path );
	exit;
}
...
if ( ! is_readable( $file_path ) ) {
	http_response_code( 500 );
	header( 'Content-Type: application/json' );
	echo json_encode( array( 'error' => 'Plugin file is not readable' ) );
	error_log( 'SocietyPress download: File not readable - ' . $file_path );
	exit;
}
...
// Log download
error_log( sprintf(
	'SocietyPress download: %s by %s from %s',
...
if ( false === $handle ) {
	http_response_code( 500 );
	error_log( 'SocietyPress download: Failed to open file - ' . $file_path );
	exit;
}
