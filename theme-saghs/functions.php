<?php
/**
 * SAGHS Child Theme Functions
 *
 * WHY: This is the bootstrap file for the SAGHS child theme. It enqueues the
 * parent stylesheet first (so the base layout loads), then the child stylesheet
 * (so SAGHS overrides win). It also registers a footer nav menu, loads the
 * Poppins font from Google Fonts, and enqueues the SAGHS-specific JavaScript
 ...
 * lives in the SocietyPress plugin. This child theme only handles SAGHS-specific
...
/**
 * Load Google Fonts, parent + child stylesheets, and SAGHS JavaScript.
 *
 ...
 * has all the base layout, forms, widgets, and user menu styles that SAGHS
 * inherits. Without enqueuing it here, the site would lose all base styling.
 *
 * Load order:
 * 1. Google Fonts (Poppins — the SAGHS brand font)
 * 2. Parent style.css (base layout, forms, widgets)
 * 3. Child style.css  (SAGHS color overrides, custom components)
 * 4. saghs.js         (hamburger, dropdowns, hero slider)
 */
add_action( 'wp_enqueue_scripts', function () {

    // Google Fonts — Poppins with weights 300–700
    // WHY Poppins: The reference SAGHS site uses Poppins as its primary font.
    ...
    // SAGHS JavaScript — hamburger menu, dropdown nav behavior, hero slider.
    // Loaded in footer (true) so it doesn't block page rendering.
    wp_enqueue_script(
        'saghs-script',
        get_stylesheet_directory_uri() . '/js/saghs.js',
        [],
        SAGHS_THEME_VERSION,
        true
    );
 ...
 *
 * WHY: SAGHS shows quick links in the footer (About Us, Contact, Library Hours).
 ...
 * for the SAGHS 3-level dropdown navigation.
 *
 ...
 */
class SAGHS_Nav_Walker extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
            $output .= '<button class="saghs-submenu-toggle" aria-expanded="false" aria-label="Toggle submenu">';
            $output .= '<span class="saghs-toggle-icon">&#9662;</span>';
            $output .= '</button>';
        }
    }
}
