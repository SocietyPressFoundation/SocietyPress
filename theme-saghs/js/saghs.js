/**
 * the society Child Theme — JavaScript
 *
...
(function () {
    ...
    function initHamburger() {
        var hamburger = document.querySelector('.society-hamburger');
    ...
    //   (added by the the society_Nav_Walker in functions.php) get click handlers
    //   that toggle the .is-open class on parent <li> elements.

    function initDropdowns() {
        var toggles = document.querySelectorAll('.society-submenu-toggle');
            ...
            toggles[i].addEventListener('click', function (e) {
                ...
                for (var j = 0; j < siblings.length; j++) {
                    if (siblings[j] !== parentLi) {
                        ...
                        var sibToggle = siblings[j].querySelector('.society-submenu-toggle');
                        if (sibToggle) {
        ...
        // a .society-submenu-toggle style rule here for clarity.
    }
    ...
    // automatically. This handles both the the society static hero (class-based
    // .society-hero) and the plugin's sp_render_builder_widget_hero_slider
    // (which has its own inline JS). We only run this if we find the the society
    // hero container with multiple slides.

    function initHeroSlider() {
        var hero = document.getElementById('society-hero');
        if (!hero) {
            return; // No the society hero on this page (maybe using plugin slider)
        }

        var slides  = hero.querySelectorAll('.society-hero-slide');
        var dots    = hero.querySelectorAll('.society-hero-dot');
        var current = 0;
